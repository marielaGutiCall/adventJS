...
export default function getMinJump(obstacles) {
   let sortObstaclesArr = obstacles.sort((a,b)=> b-a);
   let itemFound, path=[];
   let flag= false, value;
   for(let i=2; i<=sortObstaclesArr[0]; i++){
     path.push(i); 
   } 
   itemFound = (arr, value)=>{
     return arr.find(e=>value===e); 
   }
   let pathLength = path.length;
   for(let i=2; i<=pathLength; i++){
       value = i;
       if(!itemFound(sortObstaclesArr, i)){
         while(value<=path[pathLength-1]){
           value +=i;
           if(!itemFound(sortObstaclesArr, value) || value>path[pathLength-1]) flag = true;
