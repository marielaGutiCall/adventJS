...
export default function learn(time, courses) {
   let possibleValues = [], orderArray = [];
   let sum = 0;
     for(let i=0; i<courses.length; i++){
       sum = 0;
       for(let j=i+1; j<courses.length; j++){
            sum = courses[i] + courses[j];
            if(time>sum) possibleValues.push({"totalHours": sum, "indices":[i,j]})
            else {
              if(time===sum) return [i, j]
            }
        }
     }
     if(possibleValues.length > 1){
        orderArray = possibleValues.sort((a,b)=>{
          return b.totalHours - a.totalHours
        })
        possibleValues = orderArray[0].indices; 
     } else {
         if(possibleValues.length === 1) possibleValues = possibleValues[0].indices;
