...
export default function checkSledJump(heights) {
   let cont = 0, index, heightsArrLength;
   let maxHeight;
   heightsArrLength = heights.length -1
   maxHeight = heights.reduce((p,c)=>{
     return Math.max(p,c);
   })
   index = heights.indexOf(maxHeight);
   if(index!== heightsArrLength && index!== 0){
      for(let i=0; i<=index; i++){
          if(heights[i+1]>heights[i]) cont++;
      }
     for(let i=index; i<heightsArrLength; i++){
          if(heights[i]>heights[i+1]) cont++;
      }
 
     if(cont === heightsArrLength ) return true;
   }
   return false;
 }