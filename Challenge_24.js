...
export default function checkIsSameTree(treeA, treeB) {
   let flag = true;
   let compareObjs = (treeA, treeB)=>{
     let keysA = Object.keys(treeA); 
     let keysB = Object.keys(treeB);
       if(keysA.length===keysB.length && flag){
           for(let item in treeA){
             const areObjectsAB= typeof treeA[item]==='object'&&typeof treeB[item]=== 'object';
             if(treeA[item] && treeB[item] && areObjectsAB){
               compareObjs(treeA[item], treeB[item]);
             }else {
                 if(treeA[item] === treeB[item]) flag = true;
   ...
   return compareObjs(treeA, treeB);
 }