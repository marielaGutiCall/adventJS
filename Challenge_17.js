...
export default function countPackages(carriers, carrierID) {
   let itemFound = carriers.find(e=>e[0] === carrierID)
   let packages = itemFound[1];
   let verifyOrg = (carriers, arr)=>{
     let arrLength = arr.length;
     if(arrLength>0){
       for(let i=0; i<arrLength;i++){
         let itemFound = carriers.find(e=>e[0] === arr[i]);
         packages += itemFound[1];
         verifyOrg(carriers, itemFound[2])
       }
     }
   }
   verifyOrg(carriers, itemFound[2]);
