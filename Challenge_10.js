...
export default function getCoins(change) {
   ...
   let value = change, changeMin;
   coinsDupl.reverse();
   for(let i = 0; i<coinsDupl.length; i++){
        changeMin = Math.floor(value / coinsDupl[i]);
        value = value % coinsDupl[i];
        if(changeMin>0){
                changeReturned.push(changeMin) 
        } else  changeReturned.push(0) 
   }
     return changeReturned.reverse();
   }